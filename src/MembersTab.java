...
public class MembersTab {
    ...
    public static JPanel build() {
        ...
        btnEdit.addActionListener(e -> {
            ...
            if (row < 0) {
                UIHelper.msg("Please select a member to edit.");
                return;
            }
        ...
        btnDelete.addActionListener(e -> {
            int row = table.getSelectedRow();
            if (row < 0) {
                UIHelper.msg("Please select a member.");
                return;
            }
            ...
            try {
                ResultSet rs = DBHelper.conn.createStatement().executeQuery(
                        "SELECT COUNT(*) FROM transactions WHERE member_id = " + id);
                if (rs.next() && rs.getInt(1) > 0) {
                    UIHelper.msg("\"" + name + "\" \nTransactions exists .");
                    return;
                }
                if (UIHelper.confirm("Delete member: \"" + name + "\"?")) {
        ...
        btnHistory.addActionListener(e -> {
            int row = table.getSelectedRow();
            if (row < 0) {
                UIHelper.msg("Please select a member to view history.");
                return;
            }
            TransactionsTab.showMemberHistory(
                    (int) model.getValueAt(row, 0), // member id
                    (String) model.getValueAt(row, 1)); // member name
        });
    ...
    static void addMemberDialog(JFrame parent, DefaultTableModel model, JLabel statusBar) {
        ...
        form.add(UIHelper.boldLabel("Name *:"));
        form.add(tfName);
        form.add(UIHelper.boldLabel("Phone:"));
        form.add(tfPhone);
        form.add(UIHelper.boldLabel("Email:"));
        form.add(tfEmail);
        form.add(UIHelper.boldLabel("Address:"));
        form.add(tfAddress);
