...
public class BooksTab {
    ...
    public static JPanel build() {
        ...
        DefaultTableModel model = new DefaultTableModel(cols, 0) {
            public boolean isCellEditable(int r, int c) { return false; } // editing band
        };
        ...
        btnEdit.addActionListener(e -> {
            ...
            if (row < 0) { UIHelper.msg("Please select a book to edit."); return; }
        ...
        btnDelete.addActionListener(e -> {
            int row = table.getSelectedRow();
            if (row < 0) { UIHelper.msg("Please select a book."); return; }
            int id = (int) model.getValueAt(row, 0);
            if (UIHelper.confirm("Delete book: \"" + model.getValueAt(row, 1) + "\"?")) {
    ...
    static void addBookDialog(JFrame parent, DefaultTableModel model, JLabel statusBar) {
        ...
        form.add(UIHelper.boldLabel("Title *:"));  form.add(tfTitle);
        form.add(UIHelper.boldLabel("Author *:")); form.add(tfAuthor);
        form.add(UIHelper.boldLabel("Genre:"));    form.add(tfGenre);
        form.add(UIHelper.boldLabel("Year:"));     form.add(tfYear);
        ...
        saveBtn.addActionListener(e -> {
            ...
            if (tfTitle.getText().trim().isEmpty() || tfAuthor.getText().trim().isEmpty()) {
                UIHelper.msg("Title & Author are required!"); return;
            }
            int year = 0;
            try {
                ...
                UIHelper.msg("Year must be a number!"); return;
            }
