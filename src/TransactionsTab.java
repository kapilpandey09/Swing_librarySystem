...
public class TransactionsTab {
    ...
    public static JPanel build() {
        ...
        btnReturn.addActionListener(e -> {
            ...
            if (row < 0) { UIHelper.msg("Please select a transaction to return."); return; }
            if ("Returned".equals(model.getValueAt(row, 5))) {
                UIHelper.msg("This book is already returned!"); return;
            }
    ...
    public static void loadTransactions(DefaultTableModel model, JLabel statusBar) {
        ...
        try {
            String sql =
                "SELECT t.id, b.title, m.name, t.borrow_date, " +
    ...
    static void issueBookDialog(DefaultTableModel txnModel, JLabel statusBar) {
        ...
        hdr.setForeground(Color.WHITE); hdr.setOpaque(true);
        ...
        form.add(UIHelper.boldLabel("Select Member:")); form.add(memberBox);
        form.add(UIHelper.boldLabel("Select Book:"));   form.add(bookBox);
        form.add(UIHelper.boldLabel("Borrow Date:"));   form.add(dateField);
        dialog.add(form, BorderLayout.CENTER);

        JButton issueBtn = UIHelper.makeBtn("Issue Book", new Color(200, 100, 0));
        JPanel btnPanel = new JPanel(); btnPanel.add(issueBtn);
        dialog.add(btnPanel, BorderLayout.SOUTH);

        issueBtn.addActionListener(e -> {
            if (memberIds.isEmpty()) { UIHelper.msg("No members found! Add members first."); return; }
            if (bookIds.isEmpty())   { UIHelper.msg("No available books found!"); return; }
    ...
    static void returnBook(int txnId, DefaultTableModel txnModel, JLabel statusBar) {
        ...
        } catch (SQLException e) { UIHelper.msg("Error: " + e.getMessage()); }
    }
